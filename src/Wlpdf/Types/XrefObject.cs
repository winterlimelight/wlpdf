...
{
    internal class XrefObject : PdfStream, IPdfStream
    {
        ...
        public void SetEntries(IEnumerable<PdfCrossReference> entries)
        {
        ...
        public void UpdateStream()
        {
            ...
            Stream = new byte[_entries.Count * fieldWidths.Sum()];
            int inx = 0;
            foreach(PdfCrossReference xref in _entries)
            {
                ...
                Util.ByteConversions.WriteBigEndianInt((int)xref.EntryType, Stream, inx, fieldWidths[0]);
                inx += fieldWidths[0];
                Util.ByteConversions.WriteBigEndianInt(value1, Stream, inx, fieldWidths[1]);
                inx += fieldWidths[1];
                Util.ByteConversions.WriteBigEndianInt(value2, Stream, inx, fieldWidths[2]);
                inx += fieldWidths[2];
            }
        }
        ...
        private void Decode(byte [] stream)
        {
            ...
            {
                sections = (this["/Index"] as PdfArray).Select(o => (o as PdfSimple<int>).Value).ToArray();
        ...
        {
            var fieldSizeArray = this["/W"] as PdfArray; // required field, so exception on failure is fine.
            if (fieldSizeArray.Count != 3)
                throw new ArgumentException("Field size array /W must contain three values.");
            return fieldSizeArray.Select(item => (item as PdfSimple<int>).Value).ToArray();
        }
